...
class TextEditor
{
    static std::map<ErrorCode, std::string> errorCodeMap;  // список ошибок по ключам
private:
    std::string current_file;
    // содержит в себе имена файлов клиента, обновляется при запуске и при удалении файла
...
public:
    ...
    void help()
    {
        ...
        cout << "write" << '\t' << '\t' << "read" << '\t' << '\t' << "help" << '\t' << '\t' << "exit" << endl
             << endl;
    }
    ...
    void input_handler(std::string input) // обработчик ввода из терминала
    {
        ...
        else if (input == "write")
        {
            if (current_file == "NULL")
            {
                session_handler(IncorrectInput);
            }
            else
            {
                cout << "---Enter string >: ";
    ...
    static void session_handler_initializer()
    {
    ...
    void session_handler(ErrorCode errorcode) // принимает код ошибки, если ошибка фатальная вызывает run = false -> выход из программы
    {
        for (auto i = errorCodeMap.begin(); i != errorCodeMap.end(); i++)
        {
            if (i->first == errorcode)
            {
                if (i->first == 0)
                {
                    ...
                    cout << "Error code " << i->first << ". " << i->second << '.' << endl;
            }
        }
    }
};
